...
def main(config) -> None:
    ...
    train_df = extract_data(config.etl.data.train_path)
    val_df = extract_data(config.etl.data.val_path)
    test_df = extract_data(config.etl.data.test_path)
    transform_data()
    create_parent_directory(config.etl.data.path_dst)
    load_data(train_df, config.etl.data.path_dst, "train.csv")
    load_data(val_df, config.etl.data.path_dst, "val.csv")
    load_data(test_df, config.etl.data.path_dst, "test.csv")
    message = "ETL ended."
